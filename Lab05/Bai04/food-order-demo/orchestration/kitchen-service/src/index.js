...
async function connectBus() {
  try {
    ...
    await channel.bindQueue(q.queue, EXCHANGE_NAME, "KITCHEN_PREPARE_REQUEST");
    ...
    channel.consume(q.queue, (msg) => {
      ...
      if (event.type === "KITCHEN_PREPARE_REQUEST") {
        const failed = Math.random() < KITCHEN_FAIL_RATE;
        if (failed) {
          publish("KITCHEN_FAILED", {
            orderId: event.orderId,
            correlationId: event.correlationId,
            payload: {
              reason: `Random kitchen failure (${KITCHEN_FAIL_RATE})`,
            },
          });
        } else {
